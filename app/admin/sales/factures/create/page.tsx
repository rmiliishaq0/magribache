...
import { createFacture } from "@/utils/Apis"
import { useRouter } from "next/navigation"
  ...
  const {isError,isPending,mutate}=useMutation({
          mutationFn: async (data:z.infer<typeof devisSchema>) => createFacture(data),
          onSuccess: () => {
    ...
    return (
          <div className="flex flex-col gap-4 mb-4">
            <Card className="flex justify-between items-center gap-4 flex-row p-4">
              <h2 className="text-lg font-bold text-secondary">Créer Un Facture</h2>
            ...
            >
              <ResizablePanel defaultSize="40%" className="w-full">
                <div className="flex h-full  justify-center p-4 w-full">
                  <DocsForm onSubmit={onSubmit} client={clientQuery?.data?.clients ?? []} form={form} />
                </div>
              </ResizablePanel>
              <ResizableHandle />
              <ResizablePanel defaultSize="60%" className="w-full">
                <div ref={previewRef} className="flex h-full w-full justify-center p-4 w-full">
                  <DocPreview isFacture={true} name={client.name} email={client.email} phone={client.phone} form={form}/>
                </div>
              </ResizablePanel>
            </ResizablePanelGroup>  
          </div>
