...
import { PartnerPriority, PartnerSource,  } from "@/app/generated/prisma/enums";
...
import { z } from "zod";
import {fieldLabels, Options} from "@/modules/crm/constants/options-to-frensh"
import { format } from "date-fns";
import { cn } from "@/lib/utils";
import { SelectOption } from "../constants/select-option";
import { useProspectForm } from "@/modules/crm/hooks/forms/use-prospect-form";
import { crmSchema, crmSchemaWithRef } from "@/modules/crm/schemas/prospect";
import { priorityColors, statusColors } from "@/modules/crm/constants/colors";
import { useUpdateClient } from "../hooks/mutations/use-update-client";


export default function ClientInfo({client}:{client :z.infer<typeof crmSchemaWithRef>}){  
    ...
    const onSubmit = useCallback((data:z.infer<typeof crmSchema>)=>{
        mutate.mutate({...data,reference:client?.reference})
    },[mutate])
    return(
        <Card>
            <CardHeader className="flex justify-between items-center">
                <CardTitle className="text-lg font-bold text-secondary">Informations client</CardTitle>
            ...
            <FieldGroup className="p-4 grid grid-cols-2 md:grid-cols-3 gap-4 items-center justify-center">
                    ...
                    control={form.control}
                    render={({ field, fieldState }) => (
                        <Field aria-invalid={fieldState.invalid} className="md:col-span-3 col-span-2">
                ...
                :
                   <div className="grid grid-cols-2 md:grid-cols-3 gap-4 items-center justify-center">
                        {Object.entries(client).map(([key,value])=>{
                            ...
                            let isBadge= false
                            if(key =="id" || key =="activities" || key =="updatedAt" || key =="activities" || key=="type" || key=="documents" || key=="source" || key=="priority") return
